...
待办生命周期时间契约（2026-08-16 用户拍板）：`completed_at` 记录进入「已完成」的时刻（ISO 8601，与 `not_started_at` 同构：进入时写入、退出「已完成」时清除）；`expired_count` 记录进入「已过期」的累计次数（`capture transition` 手动流转与 `capture sweep` 自动扫描同源递增，只增不减，作为承诺质量的负信号——反复过期即"不是真想做"，供 review 筛清理候选）。分析口径：待办「念头诞生时刻」默认等于 `created_at`（用户通常实时想起即记录）；表达内容中自述更早诞生时间的以内容为准。本轮明确不落 `due_at`（口头相对时间如"明晚/周末"解析成本高、收益不明）与 `started_at`（"开始行动"无清晰可观测锚点，用户自身亦难回答）。

闪念按状态归档（2026-08-15 用户拍板，与待办同构；2026-08-20 补已放弃目录）：`闪念空间/` 根目录存放 `待处理` 闪念；`闪念空间/发酵中/` 存放 `发酵中`（讨论裁决不转但留档等待激发点的中间态——不转 ≠ 无价值 ≠ 处理完；可被永久卡素材检索捞取，激发后由 `permanent accept` 自动转 `已处理`）；`闪念空间/已处理/` 存放 `已处理`；`闪念空间/已放弃/` 存放 `已放弃`（裁决不值得转永久卡的归档态——归档留痕，不被永久卡素材检索捞取，也不能作为形成来源被静默复活）。`permanent accept` 接纳永久卡时把形成来源闪念（`待处理` 或 `发酵中`）自动标为 `已处理` 并移入子目录；`capture transition` 对闪念开放 `待处理`/`发酵中`/`已处理`/`已放弃` 流转，流转时自动把卡片文件移到对应状态目录并同步重写全库引用、索引与状态账本；`capture sync` 批量归位阅读层手动改状态后的闪念。全库扫描（lint/verify/review/maintain/find_note/索引）同时覆盖根目录与 `发酵中/`、`已处理/`、`已放弃/` 子目录，归档卡片不脱离系统管理。
...
标题直接使用来源名称。原文快照紧随标题，关联闪念位于原文之后；来源被闪念连接时，CLI 在关联闪念之后机械维护「连接」节（只记录双向语义连接导航，不承载新解释）。网页只永久保存规范链接，不保留带分享或追踪参数的原始链接；外部本地文档只接受 UTF-8 `.md`、`.markdown` 和 `.txt`，不保存绝对路径或伪造链接。导入 Markdown 时，开头 YAML Frontmatter 只用于提取允许的来源元数据，随后从正文快照移除；与最终来源标题相同（仅空白或引号等排版标点差异视为相同）的开头一级标题也机械移除，避免在正式来源标题下重复显示。正文开头已自带作者/来源与发布/日期信息头时，CLI 不再重复插入作者与发布日期上下文行。除此之外保持正文内容和顺序。v0.1 不接受本地 PDF 作为来源快照。
...
| `source commit` | 必须提供有实际内容的用户保存动机；纯确认文本无效。动机默认只作保存记录，**只有用户明确要求沉淀为闪念（`--flash`）才创建动机闪念**；`--attach-flash-ids` 模式只关联既有闪念 |
...
| `capture update` | 目标必须是既有轻量记录；只能更新为用户亲自提供并确认的内容；替换该类型的正文节并保留其余节——闪念＝卡片现有的正文节（单次落盘为 `原始记录`，多轮捕获为 `闪念内容`；另可用 `--section` 指定 `闪念内容`／`触发情境`／`激活逻辑`），有意思与待办＝`原始记录`；`演化记录`、`来源与论证锚点` 各有专用通道，不经 update 改写 |
...
| `connect propose` | 两端都必须是已存在的正式卡片或闪念，且类型组合在连接图（contracts.CONNECT_GRAPH）允许范围内：正式卡四类互通、闪念↔闪念、闪念↔来源；它只增加形成关系之外的可选语义连接，不承担普通永久卡片的准入激活 |
...
| `maintain source-tags` | 只设置或清除来源 Frontmatter 的可选 `tags` 字段（用户命名的高辨识度实体名；CLI 只做非空与去重弱约束，中文语境实体名、允许 AI/WAIC 等通用英文缩写，内容质量由对话层维护，2026-08-16 放宽）；空列表即清除；重跑传新值即整体替换；不触碰快照区、不触发哈希变化 |
