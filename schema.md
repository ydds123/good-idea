...
捕获会话讨论存档（2026-08-15 用户拍板）：捕获会话的 entries 同时支持 `role=user`（用户口述，进入正式闪念候选）与 `role=assistant`（Agent 输出，仅作讨论记录入档，不重置 proposal、不进入正式追溯）；`completed-captures/` 会话存档**永久保留**（`capture cleanup` 只清后台维护临时缓存，不删会话——讨论原文是认知资产）；finalize 落卡时 frontmatter 写入 `capture_session`，卡片可回溯到自己的讨论过程存档，讨论可延续而非重新开始。
