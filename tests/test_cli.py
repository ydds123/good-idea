...
class GoodIdeaCliTests(unittest.TestCase):
    ...
    def test_permanent_help_exposes_input_and_confirmation_contracts(self):
        ...
        self.assertIn("UTF-8 Markdown 草稿文件路径", propose.stdout)
        self.assertIn("UTF-8 文本文件路径", propose.stdout)
        self.assertIn("建议使用仓库外临时文件", propose.stdout)
        ...
        self.assertIn("与 --confirm-user-approved 进入同一确认门禁", accept.stdout)
        self.assertIn("新流程应使用后者表达候选形成后的再次确认", accept.stdout)
