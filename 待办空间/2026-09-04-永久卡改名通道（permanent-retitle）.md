...
| 对象 | 内容 | 说明 |
| --- | --- | --- |
| 需求类型 | 能力 | 「为什么做」区记录松海指正扩-评-收卡标题收窄本质并拍板走系统开发（B 方案）：为系统补上永久卡经用户确认改名的能力，使认知资产标题能随认识修正；也有自主成分（对自身知识系统准确性的主导权），但主导是系统能力建设；不是归属。 |
| 高阶目标 | 认知中枢 | 永久卡改名通道属认知中枢的基础设施：保证正式认知资产标题与全文引用一致性可维护；与既有同类任务（待办行动闭环、闪念 skill 优化）同挂认知中枢；不是创作能力/工具效能/行业职业。 |
| 等效性 | 中 | 主路径唯一但明确：参照 capture retitle 全链、5 项改动面已盘点、仓内参照实现完整；③契约问题（formation_draft_sha256/authoring_mode/改名记录/created_at）待拍板带来设计分叉；宪法限定内容写入只能走 CLI、无第二合法路径；不是高（路径不灵活）也不是低（参照链完整、子命令 vs 白名单扩展仍有选择空间）。 |
| 多效性 | 中 | 单一机制但双重收益：即时修正触发卡（扩-评-收卡标题与本质错位）、长期为全部永久卡提供改名通道并补全 schema 契约；只服务认知中枢一个域、不与其他机制联动；不是高（无跨机制一石多鸟）也不是低（即时修复+长期能力双收）。 |
| 成功概率 | 高 | 值不值：价值高（松海主动选 B 方案，宁开发不将就 A 方案纠偏，对系统完整性重视）；期望高（capture retitle 全链是仓内已实现参照、5 项改动面已逐项盘点、③契约项是可拍板决策非阻塞风险）；难度中（跨 service/cli/schema/测试多层、wikilink 重写与来源快照保护需谨慎但模式现成）；承诺强（当轮拍板）；不是中（无外部依赖、无未知技术风险）也不是低。 |
| 距离 | 近 | 09-04 晚聊刚拍板走 B，触发问题当前存在（扩-评-收卡标题错位待改），参照实现现成可立即开工，无搁置信号；不是中/远。 |
| 具体性 | 具体 | 卡片已列 5 项改动面、参照实现（capture retitle）、触发卡 ID 与候选标题方向、schema 增补点与测试要求，验收标准可枚举；不是模糊。 |

- 期望×价值×距离：18 分（概率 高 × 多效 中 × 距离 近）→ 优先级 P2（4 条进行中待办）
