...
| 对象 | 内容 | 说明 |
| --- | --- | --- |
| 需求类型 | 能力 | 动机是掌握视频创作技能（胜任/变强），且连着控制论实践主线——以创作为学习抓手；不是自主也非归属 |
| 高阶目标 | 创作能力 | 产出是能力谱系延伸（生图→视频）；不直接服务求职或系统建设，故不挂行业职业 |
| 等效性 | 高 | 至少四条独立路径：看成片→按镜头对提示词→看背后工程→自己复刻；任一受阻仍有余路 |
| 多效性 | 中 | 主服务创作能力单目标；"顺带深化控制论理解"是副产品而非独立目标，不到"高" |
| 成功概率 | 中 | 路径清晰、材料公开，做成概率不低；但无时间承诺、求职季精力挤占，易拖成永远排后，故非"高" |
| 距离 | 远 | 兴趣档学习项目，无时间承诺，求职季自然排后 |
| 具体性 | 具体 | 消费逻辑明确（电影→镜头→提示词→工程），路径可执行 |

- 期望×价值×距离：4 分（概率 中 × 多效 中 × 距离 远）→ 优先级 P8（9 条进行中待办）
