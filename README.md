...
用户只要求保存资料、没有发起闪念讨论时，仍使用来源流程：Agent 临时预读并询问内容相关的保存动机，用户回答后才持久化来源（保存动机作保存记录；**只有用户明确要求沉淀为闪念时才创建动机闪念**，2026-08-16 拍板）。两种场景不因输入都包含链接而混为一谈。

永久卡片必须由用户主动发起。Agent 通过一次一个问题的苏格拉底式交流帮助澄清观点；经用户授权后，可以删除口语停顿和重复、调整已有表达顺序、提炼标题，但不能增加新观点。普通永久卡片需要形成一个可独立理解、理由和边界清楚的中心判断，并确认它实际从哪些材料或表达中形成。完整草稿与形成来源确认后只能先生成候选；用户看到候选并再次明确要求创建后，才正式接纳。

普通永久卡片没有真实形成来源链接就不接纳。形成来源可以是闪念、来源、既有正式材料；若卡片直接形成于本轮口述，CLI 创建一个只含用户确认的一句形成情境和机械反链的内部见证，不保存完整聊天，也不制造闪念。完成这条最小连接后，支持、冲突、例证等额外语义关系仍是可选的，不能为了丰富网络强行造边。该门禁当前不自动扩展到母题、行动和索引卡片。
...
| `待办空间/`  | 未来需要完成的事务                  | 未开始、进行中、已过期、完成、取消 |
| `永久空间/`  | 用户确认后进入长期认知系统的卡片           | 由卡片类型决定         |

待办会按目标规划方法论自动估价（每 8 小时）：打上需求类型/高阶目标/等效性/多效性/成功概率标签并按期望×价值排出优先级，Obsidian 待办数据库直接按优先级展示，详情点进待办空间查看。

待办有 48h 行动窗口（2026-08-15 拍板）：新建默认「未开始」，48h 内开始做（改「进行中」）即通过；超 48h 未动由 `capture sweep` 自动转「已过期」移出行动清单，待你裁决——重新承诺（写一句为什么，重新计时）或干掉。摄入待办时 Agent 会追问「为什么做」（模糊才问），答案写入卡片「## 为什么做」区，估价引用它。

待办生命周期时间分析（2026-08-16 拍板）：卡片记录 `completed_at`（完成时刻，进入「已完成」写入、退出清除）与 `expired_count`（过期累计次数，只增不减）——完成周期、承诺质量（反复过期 = 不是真想做）成为可分析数据；念头诞生时刻默认等于创建时间。
...
├── log.md                              # CLI 只追加的操作日志
├── 闪念空间/
│   ├── <YYYY-MM-DD-标题>.md          # 待处理：用户的即时观点、疑问和念头
│   ├── 发酵中/
│   │   └── <YYYY-MM-DD-标题>.md      # 讨论裁决不转但留档等待激发点的中间态
│   ├── 已处理/
│   │   └── <YYYY-MM-DD-标题>.md      # 已处理闪念归档（永久卡接纳时自动移入）
│   └── 已放弃/
│       └── <YYYY-MM-DD-标题>.md      # 裁决不值得转永久卡的归档（transition 移入）
├── 溯源空间/
│   └── <YYYY-MM-DD-来源标题>.md      # 单层来源元数据、原文快照和关联闪念
├── 有意思空间/
│   └── <YYYY-MM-DD-标题>.md          # 暂时无法解释但值得保留的外部现象
├── 待办空间/
│   ├── <YYYY-MM-DD-标题>.md          # 进行中：未来需要完成的事务
│   ├── 已过期/
│   │   └── <YYYY-MM-DD-标题>.md      # 已过期待办归档（capture sweep 自动移入）
│   ├── 已完成/
│   │   └── <YYYY-MM-DD-标题>.md      # 已完成待办归档（状态流转自动移入）
│   └── 已取消/
│       └── <YYYY-MM-DD-标题>.md      # 已取消待办归档（状态流转自动移入）
├── 永久空间/
│   ├── 永久卡片/
...
| `goodidea capture update` | 用户确认后整体更新轻量记录的原始记录正文 | 是 |
| `goodidea capture retitle` | 用户确认后修改轻量记录标题并重命名文件、同步引用 | 是 |
...
- 普通永久卡片必须先满足内容标准，并有至少一个经用户确认的可寻址形成来源；只有外部依据而没有形成来源不能接纳。
- 本轮直接口述可以生成最小内部形成见证，但不能保存完整聊天或伪造闪念。
- 候选形成和正式接纳是两次独立用户决定；形成来源连接与后续可选语义连接也是两层不同关系。
- 来源更新必须先形成候选，再由用户确认。
- 快照哈希异常会阻止后续写入。
