...
一条链接的典型保存过程是：Agent 临时预读网页，向用户提出与内容相关的保存动机问题；用户回答后，CLI 在同一个事务中创建原文快照和闪念，并建立双向链接。用户不回答或放弃时，不产生持久文件。
...
| Skill | 什么时候使用 | 主要边界 |
|---|---|---|
| `goodidea-capture-flash` | 记一下想法、有意思现象或待办 | 保留用户原始表达；含链接时转来源流程 |
| `goodidea-record-literature` | 保存网页、公众号文章或其他链接资料 | 先预读和询问动机，再原子保存来源与闪念；不创建文献笔记层 |
...
| `web.py` | 规范化 URL、清理网页正文并准备 Markdown 预览 |
...
| `goodidea source preview` | 在仓库外准备来源预览 | 否 |
...
一份来源对应 `溯源空间/` 中一个 Markdown 文件。正文顺序固定为：
...
只持久化去掉分享和追踪参数后的规范链接。原始分享链接只用于当次抓取，不保存。原文快照只保留标题、作者、日期、正文和本地化图片，排除导航、广告、评论和脚本。
