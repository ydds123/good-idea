...
3. 断链、来源身份异常、派生状态镜像、索引不一致，以及正式内容残留的旧 `summary` 字段；
4. Git 工作区是否干净；
5. 孤立、矛盾、长期未行动等需要人工判断的认知健康提示。

默认不修复。机械问题获授权后可由 CLI 重建索引或更新格式；旧 `summary` 字段只能通过 `goodidea maintain metadata` 机械移除。语义连接、永久卡片接纳、判断修正仍须用户参与。快照哈希异常必须停止来源修改和相关后台维护，但不能阻止 Git 忽略的临时 `capture start/append` 保存新认知现场；正式 finalize 也不得静默修改异常来源。
