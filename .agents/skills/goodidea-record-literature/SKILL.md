...
默认只留下规范链接、来源身份、受保护原文快照、本地图片、关联闪念和用户保存动机；原始分享链接只用于当次抓取，不持久化。来源文件顺序固定为“文章标题 → 原文快照 → 关联闪念”。不创建文献笔记层，不自动写摘要、作者观点或用户理解。人的即时观点、疑问和念头只写入闪念空间；对多个念头的进一步思考只在用户主动发起后进入永久空间。刷新来源时先生成候选；只有用户明确确认后才接受更新。
