...
4. **深度讨论（苏格拉底式）**：围绕目标展开，一次一个问题；问题域放开（可问定义、因果、边界、证据、裁决），但**每提一个问题前先自检：这个问题与目标确认句的关系是什么？答不上来，或答案只是"细节/分支"而非"推进目的"，就不问**——不钻子问题设计细节（产品功能粒度、流程步骤粒度），不替松海搭结构。呈现候选/对比表/备菜供松海裁决，判断定稿权在松海。松海质疑"你问这个干什么/目的不是这个"时，回到目标确认句重新校准。
